#nlmod2 <- nls(y1 ~  (A*exp(-x1/B)-C), trace=TRUE,start=list(A=4, B=12, C=-2))

#fit <-nls(y2 ~  (A*exp(-x2/B)+C), trace=TRUE,start=list(A=14, B=6, C=2), weights=weights)
  
#A <- coef(fit)["A"]
#B <- coef(fit)["B"]
#C <- coef(fit)["C"]

#curve((A * exp(-x/B)+C), from=1, to=80, add=T, col="green")


# building a function

bleachCorrect <-    function(
                             data.object,                       # object containing the data
                             rec.frames         =   NA,         # number of frames recorded
                             bleach.begin       =    1,         # number of the startframe for the fitting (defaults to 1)
                             bleach.leaveout    =   NA,         # numerical vector of frames to leave out for fitting (where the stimulus comes) (e.g.: c(27:65))
                             #bleach.start.AS    =   NA,         # number of the frame to start fitting after the stimulus
                             weight.BS          =    8,         # how to weight the part before the stimulus
                             weight.AS          =    1,         # how to weight the part after the stimulus
                             polcol             =   "#cccccc",   # color of the weight polygon
                             mfrow              =   c(5,4),
                             fixedA             =   NA,
                             fixedB             =   NA,
                             fixedC             =   NA,  
                             ref                = "MOL_MOL",
                             estimate.B         = F,             # if TRUE, parameter B is calculated from fit to median trace of references specified in "ref", within each animal  
                             bleach.begin.ref       =    NA,         # number of the startframe for the fitting (defaults to 1)
                             bleach.leaveout.ref    =   NA,         # numerical vector of frames to leave out for fitting (where the stimulus comes) (e.g.: c(27:65))
                             weight.BS.ref          =    NA,         # how to weight the part before the stimulus
                             weight.AS.ref          =    NA,
                             diffParms4Ref      = F,             # se different fitting parameters for the reference (MOL)                                    
                             plot               = T,
                             ylim = NULL
                            )
{
...
if(is.na(bleach.begin.ref)) bleach.begin.ref <- bleach.begin
if(is.na(bleach.leaveout.ref)[1]) bleach.leaveout.ref <- bleach.leaveout
if(is.na(weight.BS.ref)) weight.BS.ref <- weight.BS
if(is.na(weight.AS.ref)) weight.AS.ref <- weight.AS

if(!is.null(ylim)) ylims <- ylim


if (is.na(fixedA) == F) A = fixedA
if (is.na(fixedB) == F) B = fixedB
if (is.na(fixedC) == F) C = fixedC
...
# define weights
    ...
    if (is.na(bleach.leaveout[1]) == F) 
        ...
        weights[(bleach.leaveout[length(bleach.leaveout)]+1):rec.frames] <- weight.AS
        }
  ...
  weights.ref <- weights
    ...
    {        
      ...
      if (is.na(bleach.leaveout.ref[1]) == F) 
          ...
          weights.ref[bleach.begin.ref:(bleach.leaveout.ref[1]-1)] <- weight.BS.ref
          weights.ref[(bleach.leaveout.ref[length(bleach.leaveout.ref)]+1):rec.frames] <- weight.AS.ref
          }         
     }
...
if (plot == T) dir.create(path=folder)
for (j in 1:length(animals))
    {
    if (plot == T) pdf(file=paste(folder,"/",animals[j],"_bleachCorrectionReport.pdf",sep=""), title=paste(animals[j],"_bleachCorrectionReport"), paper="a4", height=11.69 , width=8.27)
    if (plot == T)par(mfrow=mfrow)
    ...
    for (g in 1:length(animalx.glomeruli))
      {
      animalx.gx <- droplevels(subset(animalx, NGloTag == animalx.glomeruli[g]))
                  if (estimate.B ==T)
                    {
                    ...
                    if(is.null(ylim)) ylims <- c(min.ref,max.ref)
                  ...
                  if(PB == T) pb <- txtProgressBar(min = 0, max = dim(animalx.gx)[1], style = 3)
                  for (i in 1:dim(animalx.gx)[1]) 
                      {
                      measurement <- animalx.gx[i,]
                      measurement.data <- as.numeric(subset(measurement, select=(data0:get(paste("data",rec.frames-1,sep="")))))
                      measurement.headers <- subset(measurement, select=(-(data0:get(paste("data",rec.frames-1,sep="")))))
...
#  # plot not fitted data again, NOT WORKING YET
#  if (dim(not.fitted)[1] > 0)
#      {
#      pdf(file="notfitted_bleachCorrectionReport", paper="a4", height=11.69 , width=8.27)
#      par(mfrow=mfrow)
#        
#        not.fitted.data <- as.numeric(subset(not.fitted, select=(data0:get(paste("data",rec.frames-1,sep="")))))
#        not.fitted.headers <- subset(not.fitted, select=(-(data0:get(paste("data",rec.frames-1,sep="")))))
#        
#        for (d in 1:dim(not.fitted)[1])
#          {
#          plot(x, not.fitted.data[d,], type="l", col="darkgrey", lwd="1", ylim=c(min,max), xlab="frames", ylab="response (deltaF/F)", main=paste(not.fitted.headers$TOdour[d], "\n", " c:",not.fitted.headers$NOConc[d], " g:",not.fitted.headers$NGloTag[d],"\n",not.fitted.headers$Tanimal[d], sep=""), cex.main=.7)
#          }
#        
#      dev.off()
#      }
    
parameters <- c(rec.frames=rec.frames, bleach.begin=bleach.begin, bleach.leaveout=paste(bleach.leaveout[1],"to",bleach.leaveout[length(bleach.leaveout)]), weight.BS=weight.BS, weight.AS=weight.AS)
result <- list(bleachcorrected.measurements=all.corrected.measurements, not.fitted=not.fitted, weights=weights, parameters=parameters)
result
}
