###########################################################
############ Version 0.3 - 24.02.2009, 18:00 ##############
###########################################################
### INFO
### Function including all "Curve To Value" (CTV) functions
### 
###   ctv: number of CTV to be used
###
###   for report plotting ctv needs to supply "sig" (signal), "bg" (background) and "sig.pos" (signal position)
###
### ToDo
### 
###
### CHANGELOG 
###    23.02.2009 - added/changed report plotting function
###               - added function "which.extremum"
###    24.02.2009 - added CTV 1 for normalization
###    16.04.2010.-.added CTV "missanga1" and added support for more than one stimulus in the report
###
###########################################################

ctvs <- function(   data.object,         # your data
                    ctv.type,            # type of CTV to calculate
                    rec.frames=NA,       # number of frames recorded, will be autodetected if empty
                    bg.firstframe=NA,    # for CTVs with background subtraction this is the position of the first bg frame
                    bg.lastframe=NA,     # 2nd frame for background (window end of window)
                    sig.firstframe=NA,   # signal onset
                    sig.lastframe=NA,    # signal offset
                    wa = c(.2,.7),       # calc "width @" xx*extremum, needs 2 arguments, e.g. width when 0.2 of peakmax is reached to when peak falls to .9 of peakmax
                    plot=F,              # do a barplot in the end? (very basic plot, should change to ggplot boxplot instead)
                    report=F,            # write CTV report file (gives single traces with signal, bg ... marked and values in legend)
                    ylim=NA,             # ylim for report plots, given as 2 value vector, e.g. c(-1,5) plots from -1 to 5
                    mfrow=c(4,3),        # numer of columns and rows to plot per page
                    suffix="",           # optionally, ad describing suffix to filenames, name of data.object is used when empty
                    meanTraces=F,        # does data come from meanTraces.R?
                    fp=c(29,37,41,49)        # numerical vector of length 4 giving positions for ctv.w for putative peak on and offset
                    ) 
{
        ...
        ctv <- data.frame()
          ...
          for (i in 1:dim(ctv.data)[1])
          {
            ...
            if (tp$nturns > 0)
              ...
              if (dim(peaks)[1] > 1)                                                            # if more than one peak was detected
              {
...
#############################################################
    ...
    #assign(paste("ctv",ctv.type,sep=""), ctv)
    #ctv <- cbind(ctv.headers, get(paste("ctv",ctv.type,sep="")))
    
    ctv <- data.frame(ctv.headers, ctv)
    
    #if(ctv.type != "ctv.w") names(ctv)[which(names(ctv) == "ctv")] <- paste("ctv.",ctv.type,sep="")
    ...
    #resort temprarely in order to plot ordered by ltk
    if (ctv.type == "ltk")
    {
    ctv.backup <- ctv
