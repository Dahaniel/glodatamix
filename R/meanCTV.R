...
#'
#' @param data.object gloDatamix data.frame derived from ctvs.R
#' @param test logical, perform t.test() against control?
#' @param nullref name of the control
#' @param plev plevel to test against
  ...
  for (i in 1:length(glomeruli)) {
    ...
    for (j in 1:length(odors)) {
      ...
      for (k in 1:length(concentrations)) {
        ...
        resultx <- data.frame("glomerulus"=glomeruli[i],
                              "odor" = odors[j],
                              "concentration" = as.numeric(concentrationx$NOConc[1]), #take from concentrationx not from concentrations[k] otherwise it will be saved as factor
                              "n" = ctvs.n,
                              "mean.response" = ctvs.mean,
                              "median.response" = ctvs.median,
                              "sd" = ctvs.sd,
                              "sem" = ctvs.sem,
                              "variance" = ctvs.var,
                              "quant2.5" = ctvs.quant2.5,
                              "quant97.5" = ctvs.quant97.5,
                              "quant25" = ctvs.quant25,
                              "quant75" = ctvs.quant75,
                              "min" = ctvs.min,
                              "max" = ctvs.max,
                              "plevel" = plev,
                              "pvalue.t" = pvalue.t,
