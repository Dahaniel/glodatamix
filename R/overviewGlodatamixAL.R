###########################################################
############ Version 1.1 - 01.08.2008, 12:48 ############
###########################################################
### CHANGELOG 
### 
### 29.10.2009
###  - added glomeruli support
###
### 25.02.2009
###  - added subfolderfolder creation
###
### 29.01.2009 
###  - changed frames to rec.frames (recorded frames) as used in other functions
###
### 01.08.2008, 12:48 - v1.0.1
###  - added option to manualy set upper and lower ylim
###     - if not set, limits are calculated out of data
###     - either one or both limits can be set
###
### v1.0   15.07.2008, 15:13
###  - initial version
###
###########################################################



overviewGlodatamix <- function(data.object,rec.frames=NA,ref=NA,mol=NA,air=NA,ylim_lower=NA,ylim_upper=NA,suffix=NA,mfrow=c(4,3),glomwise=F){

### prints an pdf overview per animal with additionally all mol, air and reference measurements in one plot
### needs data.object containing data derived from "gloDatamix" function
###
### number of recorded frames, data.object from "gloDatamix" function must be defined. If mol, air and ref are not defined, the corresponding plot in the beginning will me empty.

# create folder for overviews
oldwd <- getwd()
ifelse (is.na(suffix)==T, folder <- "overviews", folder <- paste("overviews",suffix,sep="_")) 
dir.create(path=folder)
setwd(paste(oldwd,folder, sep="/"))

# if rec.frame is not defined get it from first row of dataset
if (is.na(rec.frames)){
 rec.frames <- data.object$NNoFrames[1]
 print(paste("number of frames was set to",rec.frames))
}

### get animal names and count
animals <- levels(data.object$Tanimal)


### get position of datapoints within data.frame
firstframe <- length(data.object) - rec.frames + 1
lastframe <- length(data.object)

### calculate animal overviews
i.animals <- 1:length(animals)

for (i in i.animals){

	animalx <- subset(data.object, Tanimal==animals[i])
	recordings <- levels(as.factor(animalx$NRealTime))
	glomeruli <- levels(as.factor(animalx$NGloTag))
	  color.glom <- rainbow(length(glomeruli))
	firstframex <- length(animalx) - rec.frames + 1
	lastframex <- length(animalx)
	
### calculate ylim if not set


	## old version ## ylim=c(min(animalx[,firstframex:lastframex]),max(animalx[,firstframex:lastframex]))

	if (is.na(ylim_lower)){
	 ylimlower <- min(animalx[,firstframex:lastframex])
	 }
	else ylimlower <- ylim_lower

	if (is.na(ylim_upper)){
	 ylimupper <- max(animalx[,firstframex:lastframex])
	 }
	else ylimupper <- ylim_upper
	
	ylim=c(ylimlower,ylimupper)

        if (is.na(suffix)){
	 pdf(file=paste(animals[i],"_overview.pdf",sep=""), paper="a4", height=11.69 , width=8.27)
	} else {pdf(file=paste(animals[i],"_",suffix,"_overview.pdf",sep=""), paper="a4", height=11.69 , width=8.27)}
	par(mfrow=mfrow)
	
### plot references
  if (!is.na(ref))
  {
	  refx <- subset(data.object, TOdour==ref&Tanimal==animals[i])
	  firstframe.ref <- length(refx) - rec.frames + 1
	  lastframe.ref <- length(refx)
	  j.ref <- 1:dim(refx)[1]
	  color.ref <- rainbow(length(j.ref))

	  plot(1:rec.frames, refx[1,firstframe.ref:lastframe.ref],type="n",main=refx$TOdour[1], ylim=ylim, xlab="frames", ylab="response (deltaF/F)")
	  polygon(y=c(ylim[1],ylim[1],ylim[2],ylim[2]), x=c(refx$NStim_on[1],refx$NStim_off[1],refx$NStim_off[1],refx$NStim_on[1]), density=-1, col="#efefef", border=-1)
	  polygon(y=c(ylim[1],ylim[1],ylim[2],ylim[2]), x=c(refx$Nstim2ON[1],refx$Nstim2OFF[1],refx$Nstim2OFF[1],refx$Nstim2ON[1]), density=-1, col="#efefef", border=-1)	

	  for (j in j.ref) {

		  lines(1:rec.frames, refx[j,firstframe.ref:lastframe.ref],type="l", col=color.ref[j])
		
	  }
	  if (j != 0){
	  legend("topright", legend=1:j, fill=color.ref[1:j], bty="n", ncol=2)
	  }
  }
### plot mol
  if (!is.na(mol))
  {
	  refx <- subset(data.object, TOdour==mol&Tanimal==animals[i])
	  firstframe.ref <- length(refx) - rec.frames + 1
	  lastframe.ref <- length(refx)
	  j.ref <- 1:dim(refx)[1]
	  color.ref <- rainbow(length(j.ref))

	  plot(1:rec.frames, refx[1,firstframe.ref:lastframe.ref],type="n",main=refx$TOdour[1], ylim=ylim, xlab="frames", ylab="response (deltaF/F)")
	  polygon(y=c(ylim[1],ylim[1],ylim[2],ylim[2]), x=c(refx$NStim_on[1],refx$NStim_off[1],refx$NStim_off[1],refx$NStim_on[1]), density=-1, col="#efefef", border=-1)
	  polygon(y=c(ylim[1],ylim[1],ylim[2],ylim[2]), x=c(refx$Nstim2ON[1],refx$Nstim2OFF[1],refx$Nstim2OFF[1],refx$Nstim2ON[1]), density=-1, col="#efefef", border=-1)	

	  for (j in j.ref) {

		  lines(1:rec.frames, refx[j,firstframe.ref:lastframe.ref],type="l", col=color.ref[j])
		
	  }
	  if (j != 0){
	  legend("topright", legend=1:j, fill=color.ref[1:j], bty="n", ncol=2)
	  }
	 }
### plot air
  if (!is.na(air))
  {
	  refx <- subset(data.object, TOdour==air&Tanimal==animals[i])
	  firstframe.ref <- length(refx) - rec.frames + 1
	  lastframe.ref <- length(refx)
	  j.ref <- 1:dim(refx)[1]
	  color.ref <- rainbow(length(j.ref))

	  plot(1:rec.frames, refx[1,firstframe.ref:lastframe.ref],type="n",main=refx$TOdour[1], ylim=ylim, xlab="frames", ylab="response (deltaF/F)")
	  polygon(y=c(ylim[1],ylim[1],ylim[2],ylim[2]), x=c(refx$NStim_on[1],refx$NStim_off[1],refx$NStim_off[1],refx$NStim_on[1]), density=-1, col="#efefef", border=-1)
	  polygon(y=c(ylim[1],ylim[1],ylim[2],ylim[2]), x=c(refx$Nstim2ON[1],refx$Nstim2OFF[1],refx$Nstim2OFF[1],refx$Nstim2ON[1]), density=-1, col="#efefef", border=-1)	

	  for (j in j.ref) {

		  lines(1:rec.frames, refx[j,firstframe.ref:lastframe.ref],type="l", col=color.ref[j])
		
	  }
	  if (j != 0){
	  legend("topright", legend=1:j, fill=color.ref[1:j], bty="n", ncol=2)
	  }
  }

### plot measurements:
if (glomwise == F) # plot measurementwise
{
	for (k in 1:length(recordings))
	{
		recordingx <- subset(animalx, NRealTime==recordings[k])
		plot(1:rec.frames,recordingx[1,firstframex:lastframex],type="n",main=paste(recordingx$TOdour[1],"(",recordingx$NOConc[1],"), ",recordingx$TPharma[1]), ylim=ylim, xlab="frames", ylab="response (deltaF/F)", col="red")
		polygon(y=c(ylim[1],ylim[1],ylim[2],ylim[2]), x=c(recordingx$NStim_on[1],recordingx$NStim_off[1],recordingx$NStim_off[1],recordingx$NStim_on[1]), density=-1, col="#efefef", border=-1)
		polygon(y=c(ylim[1],ylim[1],ylim[2],ylim[2]), x=c(recordingx$Nstim2ON[1],recordingx$Nstim2OFF[1],recordingx$Nstim2OFF[1],recordingx$Nstim2ON[1]), density=-1, col="#efefef", border=-1)
    for (l in 1:dim(recordingx)[1])
    {
	 	lines(1:rec.frames,recordingx[l,firstframex:lastframex],type="l", col=color.glom[l])
    }

#	 	lines(x=animalx$NStim_on[k]:animalx$NStim_off[k],y=rep(ylim[1], length(animalx$NStim_on[k]:animalx$NStim_off[k])), type="l", col="black")
#		lines(x=animalx$Nstim2ON[k]:animalx$Nstim2OFF[k],y=rep(ylim[1], length(animalx$Nstim2ON[k]:animalx$Nstim2OFF[k])), type="l", col="black")

	}
}

if (glomwise == T) # plot glomerulus and odorwise
{
  odorsx <- levels(factor(animalx$TOdour))
	for (k in 1:length(glomeruli))
	{
    for (l in 1:length(odorsx))
    {
		  glomxodorx <- subset(animalx, NGloTag==glomeruli[k] & TOdour==odorsx[l])
		  recordings <- sort(glomxodorx[,"NRealTime"])
		   col.recordings <- rainbow(length(recordings))
		  PharmaNames <- as.character(glomxodorx[glomxodorx$NRealTime==recordings,"TPharma"])
		  
		  plot(1:rec.frames,glomxodorx[1,firstframex:lastframex],type="n",main=paste("glom.:",glomeruli[k],", ",glomxodorx$TOdour[1],"(",glomxodorx$NOConc[1],")"), ylim=ylim, xlab="frames", ylab="response (deltaF/F)", col="red")
		  polygon(y=c(ylim[1],ylim[1],ylim[2],ylim[2]), x=c(glomxodorx$NStim_on[1],glomxodorx$NStim_off[1],glomxodorx$NStim_off[1],glomxodorx$NStim_on[1]), density=-1, col="#efefef", border=-1)
		  polygon(y=c(ylim[1],ylim[1],ylim[2],ylim[2]), x=c(glomxodorx$Nstim2ON[1],glomxodorx$Nstim2OFF[1],glomxodorx$Nstim2OFF[1],glomxodorx$Nstim2ON[1]), density=-1, col="#efefef", border=-1)
      for (m in 1:length(recordings))
      {
	   	  lines(1:rec.frames,glomxodorx[glomxodorx$NRealTime==recordings[m],firstframex:lastframex],type="l", col=col.recordings[m])
      }
      legend("topright", legend=PharmaNames, fill=col.recordings[1:m], bty="n", ncol=1)
    }
	}
}


	dev.off()
	
	print(paste(animals[i],"_overview.pdf......created!"))

}
setwd(oldwd)
print("..........done!")


}
